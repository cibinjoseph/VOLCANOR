...
contains
  ...
  !  real(dp) :: calclift
  !  real(dp), dimension(size(wg,1),size(wg,2)) :: gamPrev
  ...
  !  gamPrev=reshape(gamVec_prev,(/rows,cols/))
  !  do j=2,cols
  !    do i=2,rows
  ...
  !      +          (wg(i,1)%vr%gam-gamPrev(i,1))/dt
  !  enddo
  !  wg%delP=density*wg%delP

  !  do j=1,cols
  !    do i=1,rows
  !      wg(i,j)%dLift=-(wg(i,j)%delP*wg(i,j)%panelArea)*cos(wg(i,j)%alpha)
  !    enddo
  !  enddo
  ...
  !      drag2=(wg(i,j)%vr%gam-gamPrev(i,j))*wg(i,j)%panelArea*sin(wg(i,j)%alpha)/dt
  ...
  !    drag2=(wg(1,j)%vr%gam-gamPrev(1,j))*wg(1,j)%panelArea*sin(wg(1,j)%alpha)/dt
