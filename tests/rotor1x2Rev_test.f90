...
contains
  ...
  subroutine setup()
    ...
    call rotor%blade(1)%rot_pitch( &
      & sign(1._dp, rotor%Omega)*rotor%blade(1)%theta)
  ...
  subroutine test_coords()
    ...
    ! Panel (1, 1) vf
    coords(:, 1) = (/0.75_dp, 1.0_dp, 0._dp/)
    coords(:, 2) = (/-3.71826D-003, 1.0_dp, -6.59418D-002/)
