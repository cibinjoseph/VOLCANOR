...
contains
  ...
  subroutine rotor2file(rotor,row_now,filename)
    ...
    do ib=1,nb
      ...
      nx=size(rotor%blade(1)%waP,1)
      ny=rotor%ns
      write(nx_char,'(I5)') nx-(row_now-1)+1
      write(ny_char,'(I5)') ny+1
      ...
      write(10,*) 'DATAPACKING=BLOCK'
      write(10,*) 'VARLOCATION=([4]=CELLCENTERED)'!,[5]=CELLCENTERED)'
      write(10,*) ((wake_mesh(1,i,j),i=row_now,nx+1),j=1,ny+1)
      write(10,*) ((wake_mesh(2,i,j),i=row_now,nx+1),j=1,ny+1)
      write(10,*) ((wake_mesh(3,i,j),i=row_now,nx+1),j=1,ny+1)
      write(10,*) ((-1._dp*rotor%blade(ib)%waP(i,j)%vr%gam,i=row_now,nx),j=1,ny)
    enddo
