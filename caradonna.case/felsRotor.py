...
secLift = CL_nonlin*(0.5*densityFELS*secArea*vinf*vinf)
ThrustFELS = nb*np.sum(secLift)
CTFELS = ThrustFELS / (densityFELS*np.pi*radius*radius*(vTip)**2.0)
...
ax[1].plot(secSpan/radius, secCL, label='FELS')
ax[1].set_ylabel('CTx')
ax[1].grid(True)
ax[1].legend()
